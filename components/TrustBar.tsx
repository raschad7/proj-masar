...
export default function TrustBar() {
  ...
  return (
    <section ref={root} className="bg-white py-20">
      ...
      <div className="trust-marquee trust-reveal mt-10 hidden h-[72px] md:block">
        <LogoLoop
          logos={LOGOS}
          speed={60}
          direction="left"
      ...
      <div className="trust-grid mt-10 flex flex-wrap items-center justify-center gap-x-12 gap-y-8 px-6 md:hidden">
        {MUNICIPALITIES.map((m) => (
          <div
            key={`grid-${m.name}`}
            className="trust-logo group flex shrink-0 items-center gap-3 text-mutedtext transition-all duration-300 hover:-translate-y-1 hover:text-ink"
          >
            {"img" in m ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img
