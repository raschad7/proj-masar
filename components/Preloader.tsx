...
export default function Preloader() {
  ...
  useGSAP(
    () => {
      ...
      gsap.set([".pl-tile", ".pl-mark", ".pl-pulse"], {
        xPercent: -50,
        yPercent: -50,
      })
      ...
      const targets = MARKS.map((_, i) => {
        ...
        return {
          x: rand(-0.35, 0.35) * window.innerWidth,
          y: rand(-0.3, 0.3) * window.innerHeight,
        }
      })
      ...
      const coverScale =
        (Math.max(window.innerWidth, window.innerHeight) / 120) * 2.2
      tl.to(".pl-tile", {
        scale: coverScale,
        borderRadius: 0,
        duration: 0.7,
  ...
  return (
    <div
      ...
      aria-hidden
    >
      {/* Blue tile — starts as the 120px logo square, expands to cover */}
      <div
        className="pl-tile absolute left-1/2 top-1/2 h-[120px] w-[120px] bg-[#34A8D9] opacity-0"
      ...
      {MARKS.map((d, i) => (
        <svg
          key={i}
          className={`pl-mark pl-mark-${i} absolute left-1/2 top-1/2 h-[120px] w-[120px] opacity-0`}
          viewBox="0 0 56 56"
          fill="none"
